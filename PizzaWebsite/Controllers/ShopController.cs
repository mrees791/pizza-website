...
namespace PizzaWebsite.Controllers
{
    public class ShopController : BaseController
    {
        ...
        public async Task<ActionResult> Checkout()
        {
            ...
            CheckoutViewModel model = await _checkoutServices.CreateViewModelAsync(currentUser, PizzaDb,
                ListServices.DefaultQuantityList, GeographyServices.StateList,
        ...
        public async Task<ActionResult> SubmitOrder(CheckoutViewModel model)
        {
            ...
            {
                model.CartVm = await _cartServices.CreateViewModelAsync(user.ConfirmOrderCartId, PizzaDb,
                    ListServices.DefaultQuantityList);
            ...
            if (model.IsDelivery())
            {
                ...
                else
                {
                    DeliveryAddress deliveryAddress =
                        await PizzaDb.GetAsync<DeliveryAddress>(model.SelectedDeliveryAddressId);
                    if (deliveryAddress == null)
                    {
                        throw new Exception(
                            $"Delivery address with ID {model.SelectedDeliveryAddressId} does not exist.");
                    }

                    bool isAuthorized =
                        await PizzaDb.Commands.UserOwnsDeliveryAddressAsync(User.Identity.GetUserId(), deliveryAddress);
                    if (!isAuthorized)
                    {
                        throw new Exception(
                            $"User does not have authorization to access delivery address with ID {model.SelectedDeliveryAddressId}.");
                    }
        ...
        public async Task<ActionResult> Cart()
        {
            SiteUser user = await GetCurrentUserAsync();
            CartViewModel model =
                await _cartServices.CreateViewModelAsync(user.CurrentCartId, PizzaDb, ListServices.DefaultQuantityList);
            return View(model);
        }
        ...
        public async Task<ActionResult> DeleteCartItemAjax(int cartItemId)
        {
            ...
            {
                Response.StatusCode = (int) HttpStatusCode.BadRequest;
                return Json($"Current user is not allowed to modify cart item ID {cartItemId}.",
