...
namespace PizzaWebsite.Models.ViewModelServices
{
    ...
    {
        public async Task<CheckoutViewModel> CreateViewModelAsync(SiteUser user, PizzaDatabase pizzaDb,
            IEnumerable<int> quantityList, IEnumerable<State> stateList,
            IEnumerable<string> customerOrderTypeList, IEnumerable<string> deliveryAddressTypeList)
        {
            ...
            IEnumerable<StoreLocation> storeLocationList =
                await pizzaDb.GetListAsync<StoreLocation>("Name", SortOrder.Ascending, storeSearch);
            IEnumerable<DeliveryAddress> deliveryAddressList =
