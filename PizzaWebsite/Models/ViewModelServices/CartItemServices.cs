...
namespace PizzaWebsite.Models.ViewModelServices
{
    ...
    {
        public async Task<CartItemViewModel> CreateViewModelAsync(CartItemJoin cartItemJoin, PizzaDatabase pizzaDb,
        ...
        private CartItemViewModel CreateDefaultCartItemViewModel(CartItemJoin cartItemJoin,
            IEnumerable<int> quantityList)
        {
        ...
        private async Task InitializeViewModelAsync(CartPizza cartPizza, CartItemViewModel model, PizzaDatabase pizzaDb)
        {
            ...
            MenuPizzaCrustFlavor crustFlavor =
                await pizzaDb.GetAsync<MenuPizzaCrustFlavor>(cartPizza.MenuPizzaCrustFlavorId);
            ...
            if (cartPizza.ToppingList.Any())
            {
                ...
                {
                    MenuPizzaToppingType toppingType =
                        await pizzaDb.GetAsync<MenuPizzaToppingType>(topping.MenuPizzaToppingTypeId);
        ...
        private void InitializeViewModel(CartItemViewModel model, string name, string descriptionHtml,
            string modifyActionName, string modifyControllerName)
        {
            model.Name = name;
